...
fn main() -> rusqlite::Result<()> {
        ...
        .build_global()
    {
        Ok(()) => {
            ...
            info!(
                "Pula wątków Rayon została bezpiecznie zainicjalizowana. Limit: {}",
                threads_info
            );
            eprintln!(
                "{} Pula wątków Rayon: {}",
        ...
        Err(e) => {
            ...
            error!(
                "Błąd inicjalizacji puli wątków Rayon: {}. System użyje domyślnej puli OS.",
                e
            );
            eprintln!(
                "{} Nie udało się zainicjalizować globalnej puli Rayon: {}",
                "[ OSTRZEŻENIE ]".yellow().bold(),
                e
            );
            eprintln!(
                "{}",
                "System automatycznie użyje domyślnej puli wątków systemu operacyjnego."
                    .bright_black()
            );
        }
    }
    ...
    #[cfg(unix)]
    {
        ...
        if !is_root {
            tracing::warn!(
                "Brak uprawnień administratora (root) - Faza 9 (Smart Merge) może nie przywrócić właścicieli plików (chown)."
            );
            warn!(
                "Brak uprawnień administratora (root) - Faza 9 (Smart Merge) może nie przywrócić właścicieli plików (chown)."
            );
            eprintln!(
                "\n{}",
                "[OSTRZEŻENIE] Uruchomiłeś program bez uprawnień administratora (root)."
                    .yellow()
                    .bold()
            );
    ...
    ctrlc::set_handler(move || {
        ...
        if count == 0 {
            ...
            eprintln!(
                "\n{} Trwa bezpieczne zatrzymywanie wątków i I/O...",
                "[ WYKRYTO PRZERWANIE (Ctrl+C) ]".yellow().bold()
            );
            eprintln!(
                "{}",
                "(Naciśnij Ctrl+C ponownie, aby wymusić natychmiastowe twarde zamknięcie)"
                    .bright_black()
            );
            ...
            tracing::error!(
                "Podwójne Ctrl+C! Wymuszono natychmiastowe przerwanie procesu (Hard Kill)."
            );
            error!("Podwójne Ctrl+C! Wymuszono natychmiastowe przerwanie procesu (Hard Kill).");
            eprintln!(
                "\n{} Wymuszanie natychmiastowego zamknięcia procesu (Hard Kill)...",
                "[ KRYTYCZNE PRZERWANIE (Podwójne Ctrl+C) ]".red().bold()
            );
    ...
    if !db_dir.exists() {
        match fs::create_dir_all(db_dir) {
            ...
            Err(e) => {
                tracing::error!(
                    "Krytyczny błąd: nie udało się utworzyć katalogu bazy danych '{:?}': {:?}",
                    db_dir,
                    e
                );
                eprintln!(
    ...
    eprintln!(
        "{} Inicjalizacja bazy danych w ścieżce: {}",
        "[ INFORMACJA ]".cyan().bold(),
        db_path_str
    );
    ...
    let mut conn = match db::init_db(&db_path_str) {
        Ok(c) => {
            ...
            eprintln!(
                "{} Połączenie z bazą danych zostało pomyślnie zainicjalizowane.",
                "[ SUKCES ]".green().bold()
            );
            c
        }
        Err(e) => {
            tracing::error!(
                "Krytyczny błąd inicjalizacji bazy danych '{}': {:?}",
                db_path_str,
                e
            );
            error!(
            ...
            eprintln!(
                "\n{} Nie udało się zainicjalizować bazy danych w ścieżce: {}",
                "[ BŁĄD ]".red().bold(),
                db_path_str
            );
    ...
    eprintln!(
        "{} Przechodzenie do głównego menu interaktywnego...",
        "[ INFORMACJA ]".cyan().bold()
    );
    ...
    match menu::start_interactive(&mut ustawienia, &mut conn) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "\n{} Wystąpił błąd podczas działania menu interaktywnego.",
                "[ BŁĄD ]".red().bold()
            );
            eprintln!("Szczegóły błędu: {}", e);
    ...
    eprintln!(
        "{}",
        "[ 🛑 ] ZAMYKANIE WERYFIKATORA KRYMINALISTYCZNEGO"
            .red()
            .bold()
    );
    eprintln!(
    ...
    eprintln!(
        "{}",
        "Trwa zrzucanie buforów bazy danych do dysku (Checkpointing WAL)...".bright_black()
    );
    ...
    match conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE);") {
        ...
        Err(e) => {
            ...
            eprintln!(
                "{} Ostrzeżenie przy zamykaniu bazy (Checkpoint WAL): {}",
                "[ OSTRZEŻENIE ]".yellow().bold(),
                e
            );
        }
    ...
    eprintln!(
        "{}",
        "[ ✔ ] Baza danych bezpiecznie zsynchronizowana.".green()
    );
    eprintln!(
        "{}",
        "[ ✔ ] System operacyjny może teraz bezpiecznie zamknąć proces.\n".green()
    );
...
mod main {
    ...
    fn test_database_init_and_checkpoint() {
        ...
        assert!(
            ...
            "Operacja WAL checkpoint powinna zakończyć się sukcesem"
        );
