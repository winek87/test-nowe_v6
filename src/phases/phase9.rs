...
//! potrzebne. Panel boczny — patrz [`LiveStats::build_summary_block`] — jest
//! jeden, bez podziału per-źródło, analogicznie do Fazy 8.
//!
...
struct MergeCandidate {
    ...
    uid_ufs: Option<u32>,
    uid_script: Option<u32>,
    gid_ufs: Option<u32>,
    gid_script: Option<u32>,
    mode_ufs: Option<u32>,
    mode_script: Option<u32>,
    mtime_ufs: Option<i64>,
    mtime_script: Option<i64>,
    is_symlink_ufs: Option<bool>,
    is_symlink_script: Option<bool>,
    zeros_pct_ufs: Option<f64>,
    zeros_pct_script: Option<f64>,
    eof_ok_ufs: Option<bool>,
    eof_ok_script: Option<bool>,
    entropy_ufs: Option<f64>,
    entropy_script: Option<f64>,
    utf8_ok_ufs: Option<bool>,
    utf8_ok_script: Option<bool>,
    structure_ok_ufs: Option<bool>,
    structure_ok_script: Option<bool>,
    exif_ok_ufs: Option<bool>,
    exif_ok_script: Option<bool>,
    media_decoded_ufs: Option<bool>,
    media_decoded_script: Option<bool>,
    has_xattr_ufs: Option<bool>,
    has_xattr_script: Option<bool>,
    io_error_ufs: Option<bool>,
    io_error_script: Option<bool>,
    yara_match_ufs: Option<String>,
    yara_match_script: Option<String>,
    repaired_path_ufs: Option<String>,
    repaired_path_script: Option<String>,
...
pub(crate) struct LiveStats {
    /// Moment startu przebiegu — ustawiany raz w [`LiveStats::new`].
    /// Używany przez [`LiveStats::build_summary_block`] do wyliczenia
    /// prędkości MB/s i czasu trwania. Wcześniej `start_time` był osobnym
    /// argumentem przekazywanym przez `run` do `process_chunk` — trzymanie
    /// go w strukturze likwiduje jeden parametr i gwarantuje, że panel i
    /// raport końcowy liczą czas od DOKŁADNIE tego samego momentu.
    started_at: Instant,
...
impl LiveStats {
    fn new(slot_count: usize, started_at: Instant) -> Self {
        Self {
            ...
            copied_ufs_common: AtomicUsize::new(0),
            copied_script_common: AtomicUsize::new(0),
            copied_ufs_unique: AtomicUsize::new(0),
            copied_script_unique: AtomicUsize::new(0),
            copied_splice: AtomicUsize::new(0),
            symlinks_recreated: AtomicUsize::new(0),
            renamed_files: AtomicUsize::new(0),
            io_errors: AtomicUsize::new(0),
            meta_errors: AtomicUsize::new(0),
            size_verification_errors: AtomicUsize::new(0),
            repaired_used_ufs: AtomicUsize::new(0),
            repaired_used_script: AtomicUsize::new(0),
            reasons: Mutex::new(HashMap::new()),
        }
    }

    /// Buduje panel boczny "Podsumowanie na żywo" — jeden, wspólny panel
    /// bez podziału per-źródło (analogicznie do Fazy 8). Wydzielone z
    /// wolnej funkcji jako metoda, żeby stan przebiegu (liczniki + moment
    /// startu) trzymał się razem — ta sama filozofia co `Stats` z Fazy 8.
    ///
    ...
    /// rozbite — ten sam wzorzec luki co w Fazie 8.
    fn build_summary_block(&self) -> String {
        ...
        let top_reasons = {
            let map = self.reasons.lock().unwrap_or_else(|e| e.into_inner());
        ...
        format!(
            ...
            speed_mb,
            self.copied_ufs_common.load(Ordering::Relaxed),
            self.copied_script_common.load(Ordering::Relaxed),
            self.copied_splice.load(Ordering::Relaxed),
            self.copied_ufs_unique.load(Ordering::Relaxed),
            self.copied_script_unique.load(Ordering::Relaxed),
            self.symlinks_recreated.load(Ordering::Relaxed),
            self.repaired_used_ufs.load(Ordering::Relaxed),
            self.repaired_used_script.load(Ordering::Relaxed),
            ...
            self.size_verification_errors.load(Ordering::Relaxed),
            self.meta_errors.load(Ordering::Relaxed),
            top_reasons,
        )
    }
}
...
fn decide_winner(file: &MergeCandidate) -> (&'static str, &'static str) {
    if file.yara_match_ufs.is_some() && file.yara_match_script.is_none() {
        return ("script", "UFS odrzucony (Zainfekowany Malware/YARA)");
    }
    if file.yara_match_script.is_some() && file.yara_match_ufs.is_none() {
        return ("ufs", "Skrypt odrzucony (Zainfekowany Malware/YARA)");
    }
    ...
    if file.smart_splice_path.is_some() {
        return (
            "splice",
            "Złożono z dwóch uszkodzonych kopii i zweryfikowano dekodowaniem (Faza 18)",
        );
    }

    if file.repaired_path_ufs.is_some() && file.repaired_path_script.is_none() {
        return (
            "ufs",
            "Wybrano UFS (Użyto wersji fizycznie zrekonstruowanej przez system)",
        );
    }
    if file.repaired_path_script.is_some() && file.repaired_path_ufs.is_none() {
        return (
            "script",
            "Wybrano Skrypt (Użyto wersji fizycznie zrekonstruowanej przez system)",
        );
    }
    if file.repaired_path_ufs.is_some() && file.repaired_path_script.is_some() {
        return (
            "script",
            "Obydwa zrekonstruowane (Priorytet domyślny: Skrypt)",
        );
    }

    if file.in_ufs && file.in_script {
        if file.io_error_ufs == Some(true) && file.io_error_script != Some(true) {
            return ("script", "UFS odrzucony (Błąd I/O / Bad Sector)");
        }
        if file.io_error_script == Some(true) && file.io_error_ufs != Some(true) {
            return ("ufs", "Skrypt odrzucony (Błąd I/O / Bad Sector)");
        }
        if file.media_decoded_ufs == Some(false) && file.media_decoded_script != Some(false) {
            return ("script", "UFS odrzucony (Ucięty obraz / Gray Banding)");
        }
        if file.media_decoded_script == Some(false) && file.media_decoded_ufs != Some(false) {
            return ("ufs", "Skrypt odrzucony (Ucięty obraz / Gray Banding)");
        }
        if file.structure_ok_ufs == Some(false) && file.structure_ok_script != Some(false) {
            return ("script", "UFS odrzucony (Zepsuta struktura ZIP/DOCX)");
        }
        if file.structure_ok_script == Some(false) && file.structure_ok_ufs != Some(false) {
            return ("ufs", "Skrypt odrzucony (Zepsuta struktura ZIP/DOCX)");
        }
        if file.utf8_ok_ufs == Some(false) && file.utf8_ok_script != Some(false) {
            return ("script", "UFS odrzucony (Zupa binarna w pliku tekstowym)");
        }
        if file.utf8_ok_script == Some(false) && file.utf8_ok_ufs != Some(false) {
            return ("ufs", "Skrypt odrzucony (Zupa binarna w pliku tekstowym)");
        }
        if file.exif_ok_ufs == Some(false) && file.exif_ok_script != Some(false) {
            return ("script", "UFS odrzucony (Zepsute offsety EXIF)");
        }
        if file.exif_ok_script == Some(false) && file.exif_ok_ufs != Some(false) {
            return ("ufs", "Skrypt odrzucony (Zepsute offsety EXIF)");
        }
        if file.eof_ok_ufs == Some(false) && file.eof_ok_script != Some(false) {
            return ("script", "UFS odrzucony (Brak znacznika końca pliku EOF)");
        }
        if file.eof_ok_script == Some(false) && file.eof_ok_ufs != Some(false) {
            return ("ufs", "Skrypt odrzucony (Brak znacznika końca pliku EOF)");
        }

        let z_ufs = file.zeros_pct_ufs.unwrap_or(0.0);
        let z_scr = file.zeros_pct_script.unwrap_or(0.0);
        if z_ufs > 99.0 && z_scr < 99.0 {
            return ("script", "UFS odrzucony (Wydmuszka wypełniona zerami)");
        }
        if z_scr > 99.0 && z_ufs < 99.0 {
            return ("ufs", "Skrypt odrzucony (Wydmuszka wypełniona zerami)");
        }

        let e_ufs = file.entropy_ufs.unwrap_or(4.0);
        let e_scr = file.entropy_script.unwrap_or(4.0);
        if e_ufs > 7.995 && e_scr <= 7.995 {
            return ("script", "UFS odrzucony (Skrajna entropia / Szum)");
        }
        if e_scr > 7.995 && e_ufs <= 7.995 {
            return ("ufs", "Skrypt odrzucony (Skrajna entropia / Szum)");
        }

        if file.has_xattr_ufs == Some(true) && file.has_xattr_script != Some(true) {
            return ("ufs", "Wybrano UFS (Ocalił ukryte atrybuty xattr)");
        }
        if file.has_xattr_script == Some(true) && file.has_xattr_ufs != Some(true) {
            return ("script", "Wybrano Skrypt (Ocalił ukryte atrybuty xattr)");
        }

        if let (Some(size_u), Some(size_s)) = (file.size_ufs, file.size_script) {
            if size_u > size_s {
                return ("ufs", "Wybrano UFS (Większy rozmiar pliku)");
            }
            if size_s > size_u {
                return ("script", "Wybrano Skrypt (Większy rozmiar pliku)");
            }
        }

        if file.hash_match == Some(true) {
            return (
                "script",
                "Zgodne bit-do-bitu (Priorytet domyślny: Skrypt Autorski)",
            );
        }
        (
            "script",
            "Remis heurystyczny (Priorytet domyślny: Skrypt Autorski)",
        )
...
/// Kanonikalizuje ścieżkę; przy nieistniejącej ścieżce cofa się do
/// najbliższego istniejącego przodka i dokleja resztę. Gdy rodzic jest pusty
/// (ścieżka względna bez istniejącego przodka), kontynuuje od `.` (CWD).
...
/// przez `copy_file_and_meta`. Kanonikalizacja przez [`kanon`].
pub(crate) fn sciezki_bezpieczne(
    target_path: &Path,
    ufs_path: &Path,
    script_path: &Path,
...
/// faktycznym zapisem pliku inny wątek Rayon mógł w międzyczasie zarezerwować
/// dokładnie tę samą nazwę. Ostateczną, odporną na wyścig decyzję podejmuje
/// dopiero [`commit_bez_nadpisania`].
fn get_safe_target_path(
    target_base: &Path,
    rel_path: &str,
    winner: &str,
) -> (PathBuf, String, bool) {
...
fn commit_bez_nadpisania(
    tmp_path: &Path,
    target_base: &Path,
    rel_path: &str,
    winner: &str,
) -> std::io::Result<(PathBuf, String, bool)> {
    let original_path = Path::new(rel_path);
    let stem = original_path
        .file_stem()
        .and_then(|s| s.to_str())
...
/// Licznik dla [`sciezka_tymczasowa`].
...
/// żeby końcowy `fs::rename`/`hard_link` był atomowy.
fn sciezka_tymczasowa(docelowa: &Path) -> PathBuf {
    ...
    docelowa.with_file_name(format!(
        ".{}.tmp-{}-{}",
        obcieta,
        std::process::id(),
        licznik
...
/// docelowego i przywraca metadane. Jeśli zwycięska strona ma wersję
/// zrekonstruowaną przez Fazę 17, kopiuje TĘ wersję, nie oryginał.
fn copy_file_and_meta(
    file: &MergeCandidate,
    winner: &'static str,
    ufs_path: &Path,
    script_path: &Path,
    ...
    let source_path = if winner == "splice" {
        PathBuf::from(
            file.smart_splice_path
                .as_ref()
                .expect("winner=splice implikuje Some(smart_splice_path) - patrz decide_winner"),
        )
    } else if winner == "script" {
    ...
    let (expected_size, uid, gid, mode, mtime, is_symlink) = if winner == "splice" {
        (
            None,
            file.uid_script.or(file.uid_ufs),
            file.gid_script.or(file.gid_ufs),
            file.mode_script.or(file.mode_ufs),
            file.mtime_script.or(file.mtime_ufs),
            Some(false),
        )
    } else if winner == "script" {
        (
    ...
    let (mut target_path_full, mut final_rel_path, mut renamed) =
        get_safe_target_path(target_base, &file.rel_path, winner);

    if let Some(parent) = target_path_full.parent() {
    ...
    if is_sym {
        if let Ok(target_link) = fs::read_link(&source_path) {
            if std::os::unix::fs::symlink(&target_link, &target_path_full).is_err() {
                ...
                return (false, final_rel_path, zrodlo_txt);
            }
        ...
        let used_repaired = (winner == "script" && file.repaired_path_script.is_some())
            || (winner == "ufs" && file.repaired_path_ufs.is_some());
    ...
    if renamed {
        stats.renamed_files.fetch_add(1, Ordering::Relaxed);
    }

    let mut meta_err = false;
    if let Some(m_ns) = mtime {
        let sec = m_ns / 1_000_000_000;
        let nsec = (m_ns % 1_000_000_000) as u32;
        let ft = FileTime::from_unix_time(sec, nsec);
        if set_symlink_file_times(&target_path_full, ft, ft).is_err() {
            meta_err = true;
        }
    }
    if !is_sym
        && let Some(md) = mode
        && fs::set_permissions(&target_path_full, fs::Permissions::from_mode(md)).is_err()
    {
        meta_err = true;
    }
    if let (Some(u), Some(g)) = (uid, gid)
        && std::os::unix::fs::lchown(&target_path_full, Some(u), Some(g)).is_err()
    {
        meta_err = true;
    }
    if meta_err {
        stats.meta_errors.fetch_add(1, Ordering::Relaxed);
    }
...
// ============================================================================
// PRZETWARZANIE PACZKI (WĄTKI ROBOCZE)
// ============================================================================

/// Trzy ścieżki bazowe operacji Smart Merge. Grupowane w jedną strukturę,
/// żeby `process_chunk` nie miał 9 argumentów — i żeby nie dało się
/// przypadkiem podać ich w złej kolejności (wszystkie są `&Path`).
struct Sciezki<'a> {
    ufs: &'a Path,
    script: &'a Path,
    target: &'a Path,
...
#[instrument(skip(chunk, sciezki, stats, tx, tx_ui, opr_log, debug_log), fields(chunk_size = chunk.len()))]
    ...
    for file in chunk {
        ...
        if let Ok(mut f) = opr_log.lock() {
            let _ = writeln!(
                ...
                crate::utils::log_timestamp(), winner, "copy_file_and_meta", file.rel_path
            );
        }

        let call_start = debug_log.is_active().then(Instant::now);
        let (success, saved_path, source_path) = copy_file_and_meta(
            file,
            winner,
            sciezki.ufs,
            sciezki.script,
        ...
        let wynik = if success { "OK" } else { "BŁĄD I/O" };
        if let Some(t) = call_start {
            debug_log.log(winner, "copy_file_and_meta", &file.rel_path, t.elapsed(), wynik);
        }
        if let Ok(mut f) = opr_log.lock() {
            let _ = writeln!(
                f,
                "[{}] [{:<15}] [KONIEC] [Metoda: {:<24}] [Wynik: {}] Źródło: \"{}\"",
                crate::utils::log_timestamp(), winner, "copy_file_and_meta", wynik, source_path
            );
        }

        let final_reason = if success {
            reason.to_string()
        } else {
            format!("{} | BŁĄD KOPIOWANIA (Odmowa I/O)", reason)
        };
        ...
        if success {
            ...
            if winner == "splice" {
                ...
                if is_common {
                    stats.copied_script_common.fetch_add(1, Ordering::Relaxed);
                } else {
                    stats.copied_script_unique.fetch_add(1, Ordering::Relaxed);
                }
            } else {
                if is_common {
                    stats.copied_ufs_common.fetch_add(1, Ordering::Relaxed);
                } else {
                    stats.copied_ufs_unique.fetch_add(1, Ordering::Relaxed);
                }
            }

            let mut map = stats.reasons.lock().unwrap_or_else(|e| e.into_inner());
        ...
        if let Some(s) = expected_size {
            stats.processed_bytes.fetch_add(s as u64, Ordering::Relaxed);
        }
        ...
        let now = Instant::now();
        let should_update =
            current.is_multiple_of(200) || now.duration_since(last_ui_update).as_millis() > 250;
        ...
        if should_update {
            ...
            let _ = tx_ui.send(PhaseEvent::UpdateBottomPath {
                idx: bar_idx,
                path: format!("[{}] {}", winner, saved_path),
            });

            let _ = tx_ui.send(PhaseEvent::UpdateSideText {
                idx: bar_idx,
                text: stats.build_summary_block(),
            });
        }
...
/// Filtr plików jeszcze nie scalonych.
...
fn zmapuj_kandydata(row: &rusqlite::Row) -> rusqlite::Result<MergeCandidate> {
    Ok(MergeCandidate {
        ...
        in_ufs: row.get(2)?,
        in_script: row.get(3)?,
        hash_match: row.get(4)?,
        size_ufs: row.get(5)?,
        size_script: row.get(6)?,
        uid_ufs: row.get(7)?,
        uid_script: row.get(8)?,
        gid_ufs: row.get(9)?,
        gid_script: row.get(10)?,
        mode_ufs: row.get(11)?,
        mode_script: row.get(12)?,
        mtime_ufs: row.get(13)?,
        mtime_script: row.get(14)?,
        is_symlink_ufs: row.get(15)?,
        is_symlink_script: row.get(16)?,
        zeros_pct_ufs: row.get(17)?,
        zeros_pct_script: row.get(18)?,
        eof_ok_ufs: row.get(19)?,
        eof_ok_script: row.get(20)?,
        entropy_ufs: row.get(21)?,
        entropy_script: row.get(22)?,
        utf8_ok_ufs: row.get(23)?,
        utf8_ok_script: row.get(24)?,
        structure_ok_ufs: row.get(25)?,
        structure_ok_script: row.get(26)?,
        exif_ok_ufs: row.get(27)?,
        exif_ok_script: row.get(28)?,
        media_decoded_ufs: row.get(29)?,
        media_decoded_script: row.get(30)?,
        has_xattr_ufs: row.get(31)?,
        has_xattr_script: row.get(32)?,
        io_error_ufs: row.get(33)?,
        io_error_script: row.get(34)?,
        yara_match_ufs: row.get(35)?,
        yara_match_script: row.get(36)?,
        repaired_path_ufs: row.get(37)?,
        repaired_path_script: row.get(38)?,
...
/// stabilny, monotoniczny postęp niezależnie od zmian w tabeli.
fn wczytaj_strone(conn: &Connection, po_id: i32, limit: usize) -> Result<Vec<MergeCandidate>> {
    ...
    // Jawne `collect::<Result<_, _>>()` propaguje PIERWSZY błąd mapowania
    // wiersza (np. NULL tam, gdzie oczekiwano nie-NULL). Poprzednie
    // `filter_map(|r| r.ok())` cicho urywało stronę bez ostrzeżenia:
    // licznik postępu rozjeżdżał się z rzeczywistością, a `ostatni_id`
    // mógł przeskoczyć nieudany wiersz i zakończyć pętlę przedwcześnie.
...
pub fn run(
    ...
    let _ = tx_ui.send(PhaseEvent::Log(
        "Uruchomiono Fazę 9: SMART MERGE (Złota Kopia).".to_string(),
    ));

    // Ta faza FIZYCZNIE ZAPISUJE finalny wynik na dysk. Błędna konfiguracja
    // operatora (target_path == ufs_path/script_path, albo jeden zagnieżdżony
    // w drugim) fizycznie NADPISAŁABY materiał dowodowy — sprawdzamy PRZED
    // jakimkolwiek zapisem, nie po fakcie.
        ...
        && let Err(e) = fs::create_dir_all(target_path)
    {
        let _ = tx_ui.send(PhaseEvent::Log(format!(
            "BŁĄD KRYTYCZNY: Nie udało się utworzyć katalogu docelowego ({}). Sprawdź uprawnienia!",
            e
        )));
        return Ok(());
    }
    ...
    if total_files == 0 {
        let _ = tx_ui.send(PhaseEvent::Log(
            "✔ Kopiowanie Złotej Kopii (SMART MERGE) w pełni zakończone. Baza danych aktualna."
                .to_string(),
        ));
    }
    ...
    let debug_log = crate::debug_log::DebugLog::maybe_open(
        &raport_cfg.katalog,
        &crate::utils::stamp_filename("dziennik_debug_faza9.txt", &stamp),
        &config.log_level,
    );
    ...
    let opr_log = Arc::new(Mutex::new(opr_log_file));
    {
        ...
        let _ = writeln!(
            f_info,
            "=== RAPORT OPERACYJNY - FAZA 9: ZŁOTA KOPIA (SMART MERGE) ==="
        );
        let _ = writeln!(
            f_info,
            "Ślad rewizyjny (Audit Trail) wszystkich przenoszonych plików (wygrane instancje).\n"
        );
    }
    ...
    let stats = LiveStats::new(rayon::current_num_threads(), start_time);
    // Ścieżki bazowe grupowane w jedną strukturę — patrz `Sciezki`.
    let sciezki = Sciezki {
        ufs: ufs_path,
        script: script_path,
        target: target_path,
    ...
    loop {
        ...
        let wynik_zapisu: Result<()> = std::thread::scope(|s| {
            let (tx, rx): (
                mpsc::SyncSender<Vec<CopyResult>>,
                mpsc::Receiver<Vec<CopyResult>>,
            ...
            if config.io_mode == "CONCURRENT" {
                // `for_each_with` zabiera `tx` przez wartość — dropuje się
                // po powrocie, więc wątek zapisu zobaczy `Disconnected`
                // i zakończy pętlę `rx.recv_timeout`.
                strona
                ...
                // W SEQUENTIAL oryginalny `tx` NIE jest przenoszony (w
                // odróżnieniu od CONCURRENT) — `tx.clone()` w pętli daje
                // tylko klony, oryginał żyje dalej. Bez tego jawnego dropu
                // wątek zapisu nigdy nie zobaczy `Disconnected` na `rx`,
                // więc `db_thread.join()` niżej wisi w nieskończoność —
                // to deadlock cykliczny (scope czeka na join, join czeka
                // na Disconnected, Disconnected czeka na drop tx, drop tx
                // czeka na koniec scope).
                drop(tx);
            }
    ...
    if CANCEL_SIGNAL.load(Ordering::SeqCst) {
        let _ = tx_ui.send(PhaseEvent::Log(
            "🛑 Kopiowanie przerwane przez użytkownika. Zapisano dotychczasowy postęp.".to_string(),
        ));
        return Ok(());
    }
    ...
    let ufs_c = stats.copied_ufs_common.load(Ordering::SeqCst);
    let scr_c = stats.copied_script_common.load(Ordering::SeqCst);
    let ufs_u = stats.copied_ufs_unique.load(Ordering::SeqCst);
    let scr_u = stats.copied_script_unique.load(Ordering::SeqCst);
    let symlinks = stats.symlinks_recreated.load(Ordering::SeqCst);
    let renamed = stats.renamed_files.load(Ordering::SeqCst);
    let io_err = stats.io_errors.load(Ordering::SeqCst);
    let meta_err = stats.meta_errors.load(Ordering::SeqCst);
    let size_err = stats.size_verification_errors.load(Ordering::SeqCst);
    let rep_ufs = stats.repaired_used_ufs.load(Ordering::SeqCst);
    let rep_scr = stats.repaired_used_script.load(Ordering::SeqCst);
    ...
    let _ = writeln!(
        &mut log_out,
        "DZIENNIK KOŃCOWY - FAZA 9 (ZŁOTA KOPIA I SMART MERGE)"
    );
    let _ = writeln!(&mut log_out, "Czas trwania: {:.2?}", elapsed);
    let _ = writeln!(
        &mut log_out,
        "Sumaryczny transfer zapisu: {} (Średnia prędkość: {:.2} MB/s)",
    ...
    let _ = writeln!(
        &mut log_out,
        "   -> Uratowano z puli wspólnej: UFS ({}), Skrypt ({})",
        ufs_c, scr_c
    );
    let _ = writeln!(
        &mut log_out,
        "   -> Skopiowano pliki unikalne: UFS ({}), Skrypt ({})",
        ufs_u, scr_u
    );
    let _ = writeln!(
        &mut log_out,
        "   -> Wykorzystano Aktywnie Zrekonstruowane Wersje (Faza 17): UFS ({}), Skrypt ({}), razem {} plików\n",
        rep_ufs, rep_scr, repaired_used
    );

    let _ = writeln!(
        &mut log_out,
        "[ 2 ] ŚLAD REWIZYJNY (Dlaczego algorytm odrzucał/wybierał poszczególne pliki):"
    );
    let reasons_map = stats.reasons.lock().unwrap_or_else(|e| e.into_inner());
    let mut sorted_reasons: Vec<_> = reasons_map.iter().collect();
    sorted_reasons.sort_by(|a, b| b.1.cmp(a.1));
    for (reason, count) in sorted_reasons.iter() {
        let _ = writeln!(&mut log_out, "   -> {} (Ilość: {})", reason, count);
    }
    ...
    if io_err > 0 || size_err > 0 || meta_err > 0 || renamed > 0 {
        ...
        if renamed > 0 {
            let _ = writeln!(
                &mut log_out,
                "   -> Zmieniono nazwę (Ochrona przed nadpisaniem): {}",
                renamed
            );
        }
        if symlinks > 0 {
            let _ = writeln!(&mut log_out, "   -> Odtworzono symlinki: {}", symlinks);
        }
        if io_err > 0 {
            let _ = writeln!(&mut log_out, "   -> Błędy I/O zapisu: {}", io_err);
        }
        if size_err > 0 {
            let _ = writeln!(
                &mut log_out,
                "   -> Błędy weryfikacji rozmiaru po kopiowaniu: {}",
                size_err
            );
        }
    }
    ...
    info!(
        ufs_c,
        scr_c, ufs_u, scr_u, repaired_used, io_err, size_err, "Faza 9 zakończona"
    );
...
mod tests {
    ...
    fn base_candidate() -> MergeCandidate {
        MergeCandidate {
            id: 1,
            rel_path: "test.dat".to_string(),
            in_ufs: true,
            in_script: true,
            hash_match: Some(true),
            size_ufs: Some(100),
            size_script: Some(100),
            uid_ufs: None,
            uid_script: None,
            gid_ufs: None,
            gid_script: None,
    ...
    fn test_decide_yara_ufs_infected_script_wins() {
        let c = MergeCandidate {
            yara_match_ufs: Some("Trojan".to_string()),
            ..base_candidate()
        };
    ...
    fn test_decide_yara_script_infected_ufs_wins() {
        let c = MergeCandidate {
            yara_match_script: Some("Trojan".to_string()),
            ..base_candidate()
        };
    ...
    fn test_decide_yara_wins_over_repair() {
        ...
        assert_eq!(winner, "script");
        assert!(
            reason.contains("Zainfekowany"),
            "YARA musi wygrać nad rekonstrukcją: {}",
            reason
        );
    }
    ...
    fn test_decide_yara_wins_over_smart_splice() {
        ...
        assert!(
            reason.contains("Zainfekowany"),
            "YARA musi wygrać nad Smart Splice: {}",
            reason
        );
    }
    ...
    fn test_decide_structure_failure_script() {
        let c = MergeCandidate {
            structure_ok_script: Some(false),
            ..base_candidate()
        };
        assert_eq!(decide_winner(&c).0, "ufs");
    }
    ...
    // ------------------------------------------------------------------
    // decide_winner: priorytet 3 - progi ujednolicone
    // ------------------------------------------------------------------
    ...
    fn test_decide_high_entropy_ufs() {
        let c = MergeCandidate {
            entropy_ufs: Some(8.0),
            entropy_script: Some(4.0),
            ..base_candidate()
        };
        let (winner, reason) = decide_winner(&c);
        assert_eq!(winner, "script");
    ...
    fn test_safe_path_single_collision_gets_source_tag() {
        let dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        std::fs::write(dir.path().join("plik.txt"), b"istniejacy")
    ...
    fn test_safe_path_double_collision_gets_version_suffix() {
        let dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        std::fs::write(dir.path().join("plik.txt"), b"a")
            .expect("Nie można zapisać danych do pliku");
        std::fs::write(dir.path().join("plik_[SKRYPT].txt"), b"b")
            .expect("Nie można zapisać danych do pliku");
    ...
    fn test_safe_path_file_without_extension() {
        let dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        std::fs::write(dir.path().join("README"), b"a").expect("Nie można zapisać danych do pliku");
    ...
    // ------------------------------------------------------------------
    // commit_bez_nadpisania — regresja na TOCTOU
    // ------------------------------------------------------------------
    ...
    fn test_commit_bez_nadpisania_drugi_watek_nie_nadpisuje_pierwszego() {
        ...
        std::fs::write(&tmp1, b"TRESC_WATKU_A").expect("Nie można zapisać danych do pliku");
        std::fs::write(&tmp2, b"TRESC_WATKU_B").expect("Nie można zapisać danych do pliku");

        let (path1, rel1, renamed1) =
            commit_bez_nadpisania(&tmp1, dir.path(), "plik.txt", "script")
                .expect("Zatwierdzenie pliku bez nadpisywania nie powiodło się");
        let (path2, rel2, renamed2) =
            commit_bez_nadpisania(&tmp2, dir.path(), "plik.txt", "script")
                .expect("Zatwierdzenie pliku bez nadpisywania nie powiodło się");

        assert!(
            !renamed1,
            "Pierwszy 'wątek' nie miał kolizji — powinien dostać nazwę oryginalną"
        );
        assert!(
            renamed2,
            "Drugi 'wątek' musiał dostać INNĄ nazwę, bo oryginalna była już zajęta"
        );
        assert_ne!(
            path1, path2,
            "Obie kopie muszą wylądować pod różnymi ścieżkami"
        );

        assert_eq!(
            std::fs::read(&path1).expect("Nie można odczytać pliku"),
            b"TRESC_WATKU_A",
            "Treść pierwszego pliku nie może zostać nadpisana"
        );
        assert_eq!(
            std::fs::read(&path2).expect("Nie można odczytać pliku"),
            b"TRESC_WATKU_B",
            "Treść drugiego pliku musi być zachowana pod jego własną nazwą"
        );
        ...
        assert!(
            !tmp1.exists(),
            "Plik tymczasowy musi zniknąć po zatwierdzeniu"
        );
        assert!(
            !tmp2.exists(),
            "Plik tymczasowy musi zniknąć po zatwierdzeniu"
    ...
    fn test_commit_bez_nadpisania_trzecia_kolizja_dostaje_wersje_v2() {
        ...
        std::fs::write(&tmp1, b"A").expect("Nie można zapisać danych do pliku");
        std::fs::write(&tmp2, b"B").expect("Nie można zapisać danych do pliku");
        std::fs::write(&tmp3, b"C").expect("Nie można zapisać danych do pliku");

        let (_, rel1, _) = commit_bez_nadpisania(&tmp1, dir.path(), "plik.txt", "ufs")
            .expect("Zatwierdzenie pliku bez nadpisywania nie powiodło się");
        let (_, rel2, _) = commit_bez_nadpisania(&tmp2, dir.path(), "plik.txt", "ufs")
            .expect("Zatwierdzenie pliku bez nadpisywania nie powiodło się");
        let (path3, rel3, renamed3) = commit_bez_nadpisania(&tmp3, dir.path(), "plik.txt", "ufs")
            .expect("Zatwierdzenie pliku bez nadpisywania nie powiodło się");
        ...
        assert_eq!(
            std::fs::read(&path3).expect("Nie można odczytać pliku"),
            b"C"
        );
    }
    ...
    fn test_commit_bez_nadpisania_bez_kolizji_zachowuje_oryginalna_nazwe() {
        ...
        std::fs::write(&tmp, b"tresc").expect("Nie można zapisać danych do pliku");

        let (path, rel, renamed) =
            commit_bez_nadpisania(&tmp, dir.path(), "sub/plik.txt", "script")
                .expect("Zatwierdzenie pliku bez nadpisywania nie powiodło się");
        ...
        assert_eq!(
            std::fs::read(&path).expect("Nie można odczytać pliku"),
            b"tresc"
        );
        assert!(!tmp.exists());
    }
    ...
    fn test_copy_regular_file_success() {
        ...
        let target_dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        std::fs::write(ufs_dir.path().join("plik.txt"), b"zawartosc testowa")
            .expect("Nie można zapisać danych do pliku");

        let candidate = MergeCandidate {
            rel_path: "plik.txt".to_string(),
            size_ufs: Some(17),
            ..base_candidate()
        };
        let stats = LiveStats::new(rayon::current_num_threads(), Instant::now());

        let (success, saved_path, _source_path) = copy_file_and_meta(
            &candidate,
            "ufs",
            ufs_dir.path(),
            script_dir.path(),
        ...
        let copied =
            std::fs::read(target_dir.path().join("plik.txt")).expect("Nie można odczytać pliku");
        assert_eq!(copied, b"zawartosc testowa");
        assert_eq!(stats.io_errors.load(Ordering::Relaxed), 0);

        let pozostale: Vec<_> = std::fs::read_dir(target_dir.path())
        ...
        assert!(
            pozostale.is_empty(),
            "plik tymczasowy nie został posprzątany: {:?}",
            pozostale
        );
    }
    ...
    fn test_copy_bledny_rozmiar_nie_zostawia_zadnego_pliku_pod_finalna_nazwa() {
        ...
        let target_dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        std::fs::write(
            ufs_dir.path().join("plik.txt"),
            b"tresc o innej dlugosci niz oczekiwana",
        )
        .expect("Nie można zapisać danych do pliku");

        let candidate = MergeCandidate {
            rel_path: "plik.txt".to_string(),
            size_ufs: Some(999_999),
            ..base_candidate()
        };
        let stats = LiveStats::new(rayon::current_num_threads(), Instant::now());

        let (success, _, _) = copy_file_and_meta(
            &candidate,
            "ufs",
            ufs_dir.path(),
            script_dir.path(),
        ...
        assert!(!success);
        assert!(
            !target_dir.path().join("plik.txt").exists(),
            "błąd weryfikacji rozmiaru nie może zostawić pliku pod finalną nazwą"
        );
        let pozostale: Vec<_> = std::fs::read_dir(target_dir.path())
            .expect("Nie można odczytać pliku")
            .filter_map(|e| e.ok())
        ...
        assert!(
            pozostale.is_empty(),
            "katalog docelowy musi zostać pusty (bez osieroconych plików .tmp-*), znaleziono: {:?}",
            pozostale.iter().map(|e| e.file_name()).collect::<Vec<_>>()
        );
        assert_eq!(
            stats.size_verification_errors.load(Ordering::Relaxed),
            1,
            "niezgodność rozmiaru musi trafić do WŁASNEGO licznika, nie do io_errors"
        );
        assert_eq!(
            stats.io_errors.load(Ordering::Relaxed),
            0,
            "niezgodność rozmiaru NIE jest zwykłym błędem I/O"
        );
    }
    ...
    fn test_sciezki_bezpieczne_odrzuca_target_wewnatrz_ufs() {
        ...
        std::fs::create_dir_all(&target).expect("Nie można utworzyć katalogów nadrzędnych");
    ...
    fn test_sciezki_bezpieczne_odrzuca_ufs_wewnatrz_target() {
        ...
        std::fs::create_dir_all(&ufs).expect("Nie można utworzyć katalogów nadrzędnych");
    ...
    fn test_kanon_wzgledna_jednoskladnikowa_bez_istniejacego_przodka_rozwiazuje_wzgledem_cwd() {
        ...
        let oczekiwane = std::env::current_dir()
            .expect("Pobieranie bieżącego katalogu roboczego nie powiodło się")
            .join(nazwa);
        assert_eq!(
            wynik, oczekiwane,
            "ścieżka względna bez istniejącego przodka musi zostać rozwiązana względem CWD, nie zwrócona surowo"
        );
        assert!(
            wynik.is_absolute(),
            "wynik kanon() musi być zawsze bezwzględny, żeby porównania starts_with miały sens"
        );
    }

    #[test]
    fn test_kanon_wzgledna_wieloskladnikowa_bez_istniejacego_przodka_rozwiazuje_wzgledem_cwd() {
        let wynik = kanon(Path::new(
            "___test_kanon_a___/___test_kanon_b___/___test_kanon_c___",
        ));
        let oczekiwane = std::env::current_dir()
            .expect("Pobieranie bieżącego katalogu roboczego nie powiodło się")
            .join("___test_kanon_a___")
            .join("___test_kanon_b___")
            .join("___test_kanon_c___");
    ...
    fn test_kanon_katalog_istniejacy_zwraca_kanoniczna_sciezke() {
        ...
        assert_eq!(
            ...
            std::fs::canonicalize(dir.path())
                .expect("Wyznaczenie kanonicznej ścieżki nie powiodło się")
        );
    }
    ...
    fn test_kanon_lisc_nieistniejacy_pod_istniejacym_rodzicem() {
        ...
        let oczekiwane = std::fs::canonicalize(dir.path())
            .expect("Wyznaczenie kanonicznej ścieżki nie powiodło się")
            .join("jeszcze_nieutworzony_lisc");
    ...
    // ------------------------------------------------------------------
    // sciezka_tymczasowa / obetnij_do_granicy_utf8
    // ------------------------------------------------------------------
    ...
    fn test_sciezka_tymczasowa_krotka_nazwa_pozostaje_nietknieta() {
        ...
        let tmp_nazwa = tmp
            ...
            .expect("Nie można pobrać nazwy pliku");
        assert!(
            tmp_nazwa.contains("plik.jpg"),
            "krótka nazwa nie powinna być obcinana: {:?}",
            tmp_nazwa
        );
    }
    ...
    fn test_sciezka_tymczasowa_dwa_wywolania_daja_rozne_nazwy() {
        let docelowa = Path::new("/tmp/plik.jpg");
        assert_ne!(
            sciezka_tymczasowa(docelowa),
            sciezka_tymczasowa(docelowa),
            "licznik musi zapewniać unikalność nazw tymczasowych"
        );
    }
    ...
    fn test_obetnij_do_granicy_utf8_nie_tnie_w_srodku_wielobajtowego_znaku() {
        ...
        for limit in 0..=s.len() {
            ...
            assert!(
                std::str::from_utf8(wynik.as_bytes()).is_ok(),
                "wynik musi być poprawnym UTF-8 dla limitu {}",
                limit
            );
            assert!(
                wynik.len() <= limit,
                "wynik nie może przekroczyć limitu {} (jest {})",
                limit,
                wynik.len()
            );
        }
    ...
    #[test]
    fn test_sciezki_bezpieczne_wykrywa_zagniezdenie_gdy_target_jeszcze_nie_istnieje() {
        let ufs = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        let script = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        let target_nieistniejacy = ufs.path().join("jeszcze_nieutworzony_katalog_docelowy");
        assert!(
            !target_nieistniejacy.exists(),
            "test musi sprawdzać ścieżkę, która faktycznie nie istnieje"
        );

        let wynik = sciezki_bezpieczne(&target_nieistniejacy, ufs.path(), script.path());
        assert!(
            wynik.is_err(),
            "zagnieżdżenie musi zostać wykryte nawet gdy target_path jeszcze nie istnieje na dysku"
        );
    }
    ...
    fn test_sciezki_bezpieczne_akceptuje_rozlaczny_target_ktory_jeszcze_nie_istnieje() {
        let ufs = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        let script = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        let rodzic_targetu = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
    ...
    fn test_copy_splice_reads_from_smart_splice_path_not_ufs_or_script() {
        ...
        let script_dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        let target_dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        let splice_dir = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");

        std::fs::write(ufs_dir.path().join("obraz.png"), b"UFS - uszkodzona wersja")
            .expect("Nie można zapisać danych do pliku");
        std::fs::write(
            script_dir.path().join("obraz.png"),
            b"SKRYPT - inna uszkodzona wersja",
        )
        .expect("Nie można zapisać danych do pliku");
        let splice_path = splice_dir.path().join("obraz_smartsplice.png");
        std::fs::write(&splice_path, b"ZLOZONY - zweryfikowana wersja")
            .expect("Nie można zapisać danych do pliku");
        ...
        assert!(success);
        let copied =
            std::fs::read(target_dir.path().join("obraz.png")).expect("Nie można odczytać pliku");
        assert_eq!(
            copied, b"ZLOZONY - zweryfikowana wersja",
            "Powinno skopiować złożenie, nie żadną ze stron źródłowych"
        );
    }
    ...
    fn test_copy_missing_source_is_io_error() {
        ...
        let candidate = MergeCandidate {
            rel_path: "nieistniejacy.txt".to_string(),
            ..base_candidate()
        };
        let stats = LiveStats::new(rayon::current_num_threads(), Instant::now());
    ...
    fn test_copy_symlink_recreation() {
        ...
        std::fs::write(ufs_dir.path().join("cel.txt"), b"dane")
            .expect("Nie można zapisać danych do pliku");
        std::os::unix::fs::symlink("cel.txt", ufs_dir.path().join("link.txt"))
    ...
    // ------------------------------------------------------------------
    // build_summary_block (jako metoda LiveStats)
    // ------------------------------------------------------------------
    ...
    fn test_build_summary_block_reports_counts() {
        ...
        let block = stats.build_summary_block();
    ...
    /// na liście generycznych etykiet, które go celowo nie potrzebują.
    #[test]
    fn test_etykiety_maja_zarejestrowane_wyjasnienia_albo_sa_generyczne() {
        ...
        let stats = LiveStats::new(1, Instant::now());
        let block = stats.build_summary_block();
        ...
        for line in block.lines() {
            ...
            assert!(
                ...
                "etykieta '{}' z panelu Fazy 9 nie ma zarejestrowanego wyjaśnienia ani nie jest na liście generycznych",
                etykieta
            );
            sprawdzonych += 1;
        }
        assert_eq!(
            sprawdzonych, 9,
            "panel powinien mieć dokładnie 9 etykiet wymagających wyjaśnienia"
        );
    }
    ...
    fn test_sciezka_naprawiona_absolutna_jest_uzywana_wprost() {
        ...
        assert!(
            !wynik.starts_with(baza),
            "Ścieżka absolutna nie może zostać doklejona do bazy źródłowej"
        );
    }
    ...
    fn test_sciezka_naprawiona_czyta_realny_plik_z_przestrzeni_roboczej() {
        let zrodlo = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");
        let przestrzen = tempdir().expect("Nie można utworzyć katalogu tymczasowego dla testu");

        let naprawiony = przestrzen.path().join("a_repaired.txt");
        std::fs::write(&naprawiony, b"tresc naprawiona")
            .expect("Nie można zapisać danych do pliku");

        let rozwiazana = sciezka_naprawiona(
            naprawiony
                .to_str()
        ...
        assert_eq!(
            std::fs::read(&rozwiazana).expect("Nie można odczytać pliku"),
            b"tresc naprawiona"
        );
    }

    // ------------------------------------------------------------------
    // merge_source_path — ślad rewizyjny
    // ------------------------------------------------------------------
    ...
    fn test_zwraca_sciezke_zrodlowa_oryginalu_z_korpusu() {
        ...
        std::fs::write(ufs_dir.path().join("plik.txt"), b"tresc")
            .expect("Nie można zapisać danych do pliku");
        let candidate = MergeCandidate {
            rel_path: "plik.txt".to_string(),
            size_ufs: Some(5),
        ...
        let (success, _, zrodlo) = copy_file_and_meta(
            &candidate,
            "ufs",
            ufs_dir.path(),
            script_dir.path(),
    ...
    fn test_zwraca_sciezke_zrodlowa_naprawy_z_fazy17() {
        ...
        std::fs::write(ufs_dir.path().join("obraz.png"), b"ORYGINAL uszkodzony")
            .expect("Nie można zapisać danych do pliku");
        let naprawiony = przestrzen
            .path()
            .join("_phase17_repaired/ufs/obraz_repaired.png");
        ...
        .expect("Nie można utworzyć katalogów nadrzędnych");
        std::fs::write(&naprawiony, b"NAPRAWIONY").expect("Nie można zapisać danych do pliku");
        ...
        assert_eq!(
            zrodlo,
            naprawiony.to_string_lossy(),
            "Ścieżka źródłowa musi wskazywać naprawę, nie oryginał"
        );
        assert!(
            zrodlo.contains("_phase17_repaired"),
            "Po tej ścieżce diagnostyka rozpoznaje pochodzenie bajtów"
        );
        assert_eq!(
            std::fs::read(target_dir.path().join("obraz.png")).expect("Nie można odczytać pliku"),
            b"NAPRAWIONY"
    ...
    fn test_zwraca_sciezke_zrodlowa_zlozenia_z_fazy18() {
        ...
        std::fs::write(ufs_dir.path().join("obraz.png"), b"UFS")
            .expect("Nie można zapisać danych do pliku");
        std::fs::write(script_dir.path().join("obraz.png"), b"SKRYPT")
            .expect("Nie można zapisać danych do pliku");
        let splice_path = splice_dir
            .path()
            .join("_smart_splice_repaired/obraz_smartsplice.png");
        std::fs::create_dir_all(
            splice_path
                .parent()
                .expect("Nie można utworzyć katalogów nadrzędnych"),
        ...
        .expect("Nie można utworzyć katalogów nadrzędnych");
        std::fs::write(&splice_path, b"ZLOZONY").expect("Nie można zapisać danych do pliku");
        ...
        assert!(success);
        assert!(
            zrodlo.contains("_smart_splice_repaired"),
            "dostałem: {}",
            zrodlo
        );
    }
    ...
    fn test_sciezka_zrodlowa_jest_zwracana_takze_przy_bledzie_kopiowania() {
        ...
        assert!(
            !success,
            "Kopiowanie nieistniejącego pliku musi się nie udać"
        );
        assert_eq!(zrodlo, ufs_dir.path().join("nie_ma.txt").to_string_lossy());
    }

    // ------------------------------------------------------------------
    // STRONICOWANIE (wczytaj_strone)
    // ------------------------------------------------------------------
    ...
    fn test_wczytaj_strone_respektuje_limit_i_kolejnosc() {
        ...
        let strona = wczytaj_strone(&conn, 0, 3).expect("Wczytanie strony danych nie powiodło się");
        ...
        assert_eq!(
            identyfikatory,
            vec![1, 2, 3],
            "Rekordy muszą iść po rosnącym id"
        );
    }
    ...
    fn test_wczytaj_strone_pomija_juz_scalone() {
        ...
        conn.execute("UPDATE files SET phase9_done = 1 WHERE id IN (1, 2)", [])
            .expect("Wykonanie zapytania SQL na bazie danych nie powiodło się");

        let strona =
            wczytaj_strone(&conn, 0, 10).expect("Wczytanie strony danych nie powiodło się");

        let identyfikatory: Vec<i32> = strona.iter().map(|k| k.id).collect();
        assert_eq!(
            identyfikatory,
            vec![3, 4, 5],
            "Scalone rekordy nie mogą wrócić"
        );
    }
    ...
    fn test_stronicowanie_kluczem_nie_przeskakuje_rekordow() {
        ...
        loop {
            let strona = wczytaj_strone(&conn, ostatni_id, 3)
                .expect("Wczytanie strony danych nie powiodło się");
            if strona.is_empty() {
                break;
            }
            ostatni_id = strona[strona.len() - 1].id;
            ...
            for id in &ids {
                conn.execute(
                    "UPDATE files SET phase9_done = 1 WHERE id = ?1",
                    params![id],
                )
                .expect("Wykonanie zapytania SQL na bazie danych nie powiodło się");
            }
        ...
        assert_eq!(
            ...
            "Każdy rekord musi zostać zwrócony DOKŁADNIE raz"
        );
    }
    ...
    fn test_wczytaj_strone_konczy_sie_na_pustej_stronie() {
        ...
        assert!(
            wczytaj_strone(&conn, 2, 10)
                .expect("Wczytanie strony danych nie powiodło się")
                .is_empty(),
            "Za ostatnim id nie ma już nic"
        );
    }
    ...
    fn test_stronicowanie_posuwa_sie_takze_gdy_rekord_nie_zostal_oznaczony() {
        ...
        let pierwsza =
            wczytaj_strone(&conn, 0, 2).expect("Wczytanie strony danych nie powiodło się");
        let ostatni_id = pierwsza[pierwsza.len() - 1].id;
        let druga =
            wczytaj_strone(&conn, ostatni_id, 2).expect("Wczytanie strony danych nie powiodło się");

        let ids: Vec<i32> = druga.iter().map(|k| k.id).collect();
        assert_eq!(
            ids,
            vec![3, 4],
            "Druga strona musi ruszyć dalej, nie powtórzyć pierwszej"
        );
    }
    ...
    fn test_kolumny_kandydata_zgadzaja_sie_z_mapowaniem() {
        ...
        let strona = wczytaj_strone(&conn, 0, 1).expect("Wczytanie strony danych nie powiodło się");
    ...
    // ------------------------------------------------------------------
    // TEST INTEGRACYJNY w `mod tests` (dla kompatybilności)
    // ------------------------------------------------------------------
    ...
    fn test_przebieg_kopiuje_wszystko_i_konczy_petle_stron() {
        ...
        std::fs::create_dir_all(ufs.path().join("dane"))
            .expect("Nie można utworzyć katalogów nadrzędnych");
        for i in 1..=5 {
            std::fs::write(ufs.path().join(format!("dane/plik{}.dat", i)), tresc)
                .expect("Nie można zapisać danych do pliku");
        }
        ...
        assert_eq!(
            niescalone, 0,
            "Po przebiegu nie może zostać ani jeden nieprzetworzony rekord"
        );
        ...
        for i in 1..=5 {
            ...
            assert!(
                docelowy.exists(),
                "brak pliku w Złotej Kopii: {}",
                docelowy.display()
            );
            assert_eq!(
                std::fs::read(&docelowy).expect("Nie można odczytać pliku"),
                tresc
            );
        }
        ...
        let (zrodlo, sciezka_zrodlowa): (String, String) = conn.query_row(
            ...
            [], |r| Ok((r.get(0)?, r.get(1)?))).expect("Wykonanie procedury głównej nie powiodło się");
        assert_eq!(
            zrodlo, "ufs",
            "Plik unikalny dla UFS musi mieć UFS jako źródło"
        );
        assert!(
            sciezka_zrodlowa.starts_with(&ufs.path().to_string_lossy().to_string()),
            "merge_source_path musi wskazywać realne źródło: {}",
            sciezka_zrodlowa
        );
    }
    ...
    fn test_przebieg_na_bazie_bez_kandydatow_konczy_sie_od_razu() {
        ...
        let mut conn =
            crate::db::init_db(":memory:").expect("Inicjalizacja bazy danych nie powiodła się");
        conn.execute(
            "INSERT INTO files (relative_path, phase9_done) VALUES ('gotowe.dat', 1)",
            [],
        )
        .expect("Wykonanie zapytania SQL na bazie danych nie powiodło się");
    ...
    fn test_blok_zawiera_znacznik_aktywnosci_watkow() {
        let stats = LiveStats::new(2, Instant::now());
...
mod integration_tests {
    ...
    /// Bez tego równoległe testy anulowania mogłyby się wzajemnie sabotować.
    ...
    impl TempDir {
        fn new(tag: &str) -> Self {
            ...
            let nanos = std::time::SystemTime::now()
                ...
                .as_nanos();
            let p = std::env::temp_dir().join(format!("phase9_test_{}_{}_{}", tag, pid, nanos));
    ...
    /// `raporty_faz` czyszczone — domyślnie kieruje raporty do `./dziennik/fazy`.
    /// `io_mode = "SEQUENTIAL"` — determinizm w testach wielowątkowych.
    fn config_seq(ufs: &Path, script: &Path, target: &Path, logs: &Path) -> Ustawienia {
        let mut u = Ustawienia {
    ...
    fn insert_unique_ufs(conn: &Connection, rel: &str, size: i64) {
        conn.execute(
            "INSERT INTO files (relative_path, found_in_ufs, found_in_script, size_ufs)
             VALUES (?1, 1, 0, ?2)",
            params![rel, size],
    ...
    fn test_run_empty_db_completes_cleanly() {
        ...
        let target = TempDir::new("empty_target");
        let logs = TempDir::new("empty_logs");

        let mut conn = fresh_db();
        let config = config_seq(ufs.path(), script.path(), target.path(), logs.path());
        let (tx_ui, _rx_ui) = mpsc::channel();

        CANCEL_SIGNAL.store(false, Ordering::SeqCst);
        run(&mut conn, &config, tx_ui).expect("run() na pustej bazie");

        let count = std::fs::read_dir(target.path()).expect("read_dir").count();
        assert_eq!(count, 0, "pusty cel po pustym przebiegu");
    }

    #[test]
    fn test_run_copies_unique_file_from_ufs_and_marks_done() {
        let ufs = TempDir::new("copy_ufs");
        let script = TempDir::new("copy_script");
        let target = TempDir::new("copy_target");
        let logs = TempDir::new("copy_logs");

        let content = b"tresc testowa";
        std::fs::create_dir_all(ufs.path().join("dane")).expect("mkdir");
        std::fs::write(ufs.path().join("dane/plik.txt"), content).expect("write");

        let mut conn = fresh_db();
        insert_unique_ufs(&conn, "dane/plik.txt", content.len() as i64);
        ...
        let target_file = target.path().join("dane/plik.txt");
        assert!(target_file.exists(), "plik musi być w Złotej Kopii");
        assert_eq!(std::fs::read(&target_file).expect("read"), content);

        let (done, source): (i64, String) = conn
            .query_row(
                "SELECT phase9_done, merge_source FROM files WHERE relative_path = 'dane/plik.txt'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .expect("query_row");
        assert_eq!(done, 1);
        assert_eq!(source, "ufs");
    }

    #[test]
    fn test_run_marks_done_even_when_source_missing() {
        let ufs = TempDir::new("missing_ufs");
        let script = TempDir::new("missing_script");
        let target = TempDir::new("missing_target");
        let logs = TempDir::new("missing_logs");

        let mut conn = fresh_db();
        insert_unique_ufs(&conn, "nie_ma.txt", 100); // brak fizycznego pliku

        let config = config_seq(ufs.path(), script.path(), target.path(), logs.path());
        let (tx_ui, _rx_ui) = mpsc::channel();

        CANCEL_SIGNAL.store(false, Ordering::SeqCst);
        run(&mut conn, &config, tx_ui).expect("run() mimo błędu kopiowania");

        let (done, success): (i64, i64) = conn
            .query_row(
                "SELECT phase9_done, merge_success FROM files WHERE relative_path = 'nie_ma.txt'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .expect("query_row");
        assert_eq!(
            ...
            "rekord musi być oznaczony jako przetworzony, inaczej następny przebieg weźmie go ponownie"
        );
        assert_eq!(success, 0, "merge_success=0 przy nieudanym kopiowaniu");
    }

    #[test]
    fn test_run_copies_from_smart_splice_path_when_present() {
        let ufs = TempDir::new("splice_ufs");
        let script = TempDir::new("splice_script");
        let target = TempDir::new("splice_target");
        let logs = TempDir::new("splice_logs");
        let splice_dir = TempDir::new("splice_src");

        std::fs::write(ufs.path().join("obraz.png"), b"UFS broken").expect("write");
        std::fs::write(script.path().join("obraz.png"), b"SCRIPT broken").expect("write");
        let splice = splice_dir.path().join("obraz_smartsplice.png");
        std::fs::write(&splice, b"SPLICE OK").expect("write");

        let mut conn = fresh_db();
        conn.execute(
            "INSERT INTO files (relative_path, found_in_ufs, found_in_script, smart_splice_path)
             VALUES ('obraz.png', 1, 1, ?1)",
            params![splice.to_string_lossy().to_string()],
        )
        .expect("INSERT");
        ...
        let target_file = target.path().join("obraz.png");
        assert!(target_file.exists());
        assert_eq!(std::fs::read(&target_file).expect("read"), b"SPLICE OK");

        let source: String = conn
            .query_row(
                "SELECT merge_source FROM files WHERE relative_path = 'obraz.png'",
                [],
                |r| r.get(0),
            )
            .expect("query_row");
        assert_eq!(source, "splice");
    }

    #[test]
    fn test_run_skips_already_done_candidates() {
        let ufs = TempDir::new("skip_ufs");
        let script = TempDir::new("skip_script");
        let target = TempDir::new("skip_target");
        let logs = TempDir::new("skip_logs");

        std::fs::write(ufs.path().join("b.txt"), b"bbb").expect("write");

        let mut conn = fresh_db();
        insert_unique_ufs(&conn, "a.txt", 3);
        conn.execute(
            "UPDATE files SET phase9_done = 1 WHERE relative_path = 'a.txt'",
            [],
        )
        .expect("UPDATE");
        insert_unique_ufs(&conn, "b.txt", 3);
        ...
        let done_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM files WHERE phase9_done = 1",
                [],
                |r| r.get(0),
            )
            .expect("query_row");
        assert_eq!(done_count, 2, "stary done + nowy przetworzony");

        let a_source: Option<String> = conn
            .query_row(
                "SELECT merge_source FROM files WHERE relative_path = 'a.txt'",
                [],
                |r| r.get(0),
            )
            .expect("query_row");
        assert!(
            a_source.is_none(),
            "run() nie może ruszyć rekordów phase9_done=1, znaleziono merge_source={:?}",
            a_source
        );
    }

    #[test]
    fn test_run_blocks_when_target_inside_source() {
        let ufs = TempDir::new("nested_ufs");
        let script = TempDir::new("nested_script");
        let logs = TempDir::new("nested_logs");

        // target WEWNĄTRZ ufs — powinno zostać zablokowane przez `sciezki_bezpieczne`.
        let target = ufs.path().join("podkatalog_zloty");
        std::fs::create_dir_all(&target).expect("mkdir");

        let mut conn = fresh_db();
        insert_unique_ufs(&conn, "cokolwiek.txt", 10);

        let config = config_seq(ufs.path(), script.path(), &target, logs.path());
        let (tx_ui, _rx_ui) = mpsc::channel();

        CANCEL_SIGNAL.store(false, Ordering::SeqCst);
        run(&mut conn, &config, tx_ui).expect("run() zwraca Ok, ale loguje błąd konfiguracji");

        let done: Option<i64> = conn
            .query_row(
                "SELECT phase9_done FROM files WHERE relative_path = 'cokolwiek.txt'",
                [],
                |r| r.get(0),
            )
            .expect("query_row");
        assert!(
            done.is_none() || done == Some(0),
            "przy zablokowanej konfiguracji nic nie może być przetworzone, phase9_done={:?}",
            done
        );

        let count = std::fs::read_dir(&target).expect("read_dir").count();
        assert_eq!(
            count, 0,
            "przy zablokowanej konfiguracji nie może powstać żaden plik w target"
        );
    }
    ...
    fn test_run_honors_cancel_signal_and_reports_partial_progress() {
        ...
        let target = TempDir::new("cancel_target");
        let logs = TempDir::new("cancel_logs");

        // 500 plików × 100 bajtów — run() trwa wystarczająco długo, żeby
        // cancel zdążył zaskoczyć w środku.
        const TOTAL: usize = 500;
        let content = vec![b'x'; 100];
        let content_len = content.len() as i64;
        for i in 0..TOTAL {
            std::fs::write(ufs.path().join(format!("plik{:03}.txt", i)), &content).expect("write");
        }
        ...
        let mut conn = fresh_db();
        {
            let mut stmt = conn
                .prepare(
                    "INSERT INTO files (relative_path, found_in_ufs, size_ufs) VALUES (?1, 1, ?2)",
                )
                .expect("prepare");
            for i in 0..TOTAL {
                stmt.execute(params![format!("plik{:03}.txt", i), content_len])
                    .expect("INSERT");
            }
        }
        ...
        // Cancel-wątek: czeka aż w `target` pojawią się 3 pliki (dowód, że
        // run() już coś skopiował), potem ustawia sygnał.
        let canceller = thread::spawn({
            ...
            move || {
                ...
                loop {
                    ...
                    let count = std::fs::read_dir(&target_path)
        ...
        assert!(
            ...
            "po anulowaniu powinno być trochę przetworzonych rekordów, jest {}",
            done
        );
        assert!(
            done < TOTAL as i64,
            "po anulowaniu nie wszystko powinno być przetworzone (jest {} z {})",
            done,
            TOTAL
        );
    }
